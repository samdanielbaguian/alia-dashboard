  ...
  useEffect(() => {
    const fetchSellers = async () => {
      try {
        ...
        // Fetch all products to extract unique sellers/merchants
        ...
        // Extract unique merchant IDs from products
        const sellerMap = {};
        ...
        products.forEach(product => {
          ...
          if (sellerId && !sellerMap[sellerId]) {
            sellerMap[sellerId] = {
              ...
              products: 0,
              sales: 0,
              rating: (Math.random() * 2 + 3.5).toFixed(1), // Mock rating 3.5-5.5
              status: 'active',
        ...
        // Count products and calculate sales per seller
        products.forEach(product => {
          const sellerId = product.merchant_id;
        ...
        // Fetch individual merchant details for each seller
        const sellersWithDetails = await Promise.all(
          Object.values(sellerMap).map(async (seller) => {
            try {
              ...
              // If individual merchant fetch fails, use aggregated data
              return {
                ...seller,
                products: seller.products,
                sales: parseFloat(seller.sales.toFixed(2)),
        ...
        setStats({
          ...
          newThisMonth: Math.floor(sellersWithDetails.length * 0.1), // Mock: 10% are new
        });
        setError(null);
      } catch (err) {
